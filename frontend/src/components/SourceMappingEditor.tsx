...
export function SourceMappingEditor({source,onSaved}:{source:string;onSaved:()=>void}) {
 ...
 async function open(){setBusy(true);setMessage('');setReview(false);try{
  ...
  setIpPolicy(value.ip_conflict_policy==='observe'?'observe':'strict');setState(value);setDraft(d=>({...d,references:value.references,host_types:value.host_types}));
