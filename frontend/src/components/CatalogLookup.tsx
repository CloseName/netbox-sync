...
export function Lookup({kind,label,value,change,language,hint='',host,onCreate,assessment}:{kind:string;label:string;value?:CatalogItem;change:(r:CatalogItem)=>void;language:string;hint?:string;host?:HostPreview;onCreate?:()=>void;assessment?:(row:CatalogItem)=>string}){
 ...
 const context=(row:CatalogItem)=>[row.manufacturer?.name,row.type?.name,row.scope?.name||row.site?.name].filter(Boolean).join(' · ');
 const close=()=>{setOpen(false);control.current?.focus();};
 const choose=(row:CatalogItem)=>{if(assessment?.(row))return;change({...row,suggested:false});close();};
 return <section className="catalog-lookup" ref={root} onBlur={e=>{if(e.relatedTarget&&!e.currentTarget.contains(e.relatedTarget as Node))setOpen(false);}}>
 ...
 {value&&assessment?.(value)&&<small role="alert">{assessment(value)}</small>}
 ...
 <ul id={id+'-list'} role="listbox" aria-label={label} className="catalog-options">{page?.items.map((row,index)=><li id={id+'-option-'+index} role="option" aria-selected={row.id===value?.id} aria-disabled={!!assessment?.(row)} key={row.id} data-catalog-id={row.id} className={active===index?'is-active':''} onMouseDown={e=>e.preventDefault()} onClick={()=>choose(row)}>{row.name}<small>{context(row)}</small>{assessment?.(row)&&<small>{assessment(row)}</small>}</li>)}</ul>
