...
export function CatalogCreate({kind,initial,manufacturer='',references,language,onClose,onCreated}:{kind:string;initial:string;manufacturer?:string;references:Record<string,CatalogItem>;language:string;onClose:()=>void;onCreated:(row:CatalogItem)=>void}){
 ...
 {initial&&<p className="muted">{kind==='device_type'?t('Suggested name — review it. Hardware descriptions may be generic; no height or SKU is inferred.','Предложенное имя — проверьте его. Описание оборудования может быть общим; высота и SKU не определяются предположением.'):t('Review and edit the suggested name.','Проверьте и при необходимости измените предложенное имя.')}</p>}
