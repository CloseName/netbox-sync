...
export function SourceLifecyclePanel({ source, onRemoved }: { source: Source; onRemoved: (value: Lifecycle)=>void }) {
  ...
  const remove=async()=>{
    if(busy || typed!==state.data?.display_name || !state.data?.revision) return;
  ...
  return <section className="source-panel"><h3>{tr("Lifecycle")}{" "}</h3>
    ...
    <dialog ref={dialog} className="sync-dialog" onCancel={event=>{event.preventDefault();if(!busy)close();}} aria-labelledby="remove-title">
      ...
      <label htmlFor="remove-source-id">{tr("Type the exact display name")}{" "}</label><input id="remove-source-id" value={typed} onChange={e=>setTyped(e.target.value)} autoComplete="off" disabled={busy} />
      {error && <p role="alert">{tr(error)} <Link to={sourcePath(source.source_instance)+'/runs'}>{tr("Review run history")}{" "}</Link></p>}
      <div className="page-actions"><button ref={cancel} disabled={busy} onClick={close}>{tr("Cancel")}{" "}</button><button className="danger" disabled={busy || typed!==state.data?.display_name || !state.data?.revision || !!state.error} onClick={remove}>{busy?tr("Removing source..."):tr("Remove Source")}</button></div>
    </dialog>
  </section>;
}
