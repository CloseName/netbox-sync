...
export async function authRequest(path:string,body?:unknown){
 ...
 if(!response.ok)throw new Error(['API_VALIDATION_FAILED','AUTH_REQUIRED','AUTH_INVALID','AUTH_RATE_LIMITED','AUTH_DENIED','AUTH_UNAVAILABLE','ENROLLMENT_INVALID','POLICY_CONFLICT','POLICY_HOST_MANAGED','POLICY_INVALID','LDAP_INVALID','LDAP_TLS_FAILED','LDAP_UNAVAILABLE','LDAP_BIND_FAILED','LDAP_ACCESS_DENIED','LDAP_CONFLICT'].includes(data.error?.code)?data.error.code:'AUTH_UNAVAILABLE');
 return data;
}
