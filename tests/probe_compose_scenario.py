...
image = 'netbox-sync-probe:review'
...
overlay.write_text(json.dumps({'services': {'netbox-sync-probe-worker': {'volumes': [
...
c.request('POST',payload['path'],json.dumps(payload['body']),headers={'Content-Type':'application/json','Host':'sync.example.test','Origin':'https://sync.example.test','X-Forwarded-Proto':'https','X-NetBox-Sync-CSRF':'same-origin'})
r=c.getresponse();print(json.dumps({'status':r.status,'body':json.loads(r.read())}))
"""
def request(body, path='/api/v1/sources/test-connection'):
...
if pgmode == 'bundled':
    # Reproduce the old, real in-API subprocess path with its unchanged DB-only network.
    old = root / 'state/old-probe.yml'
    old.write_text('services:\n  netbox-sync-api:\n    environment:\n      NETBOX_SYNC_PROBE_SOCKET: ""\n')
    run([*command,'-f',str(old),'up','-d','--no-deps','netbox-sync-api'])
    for _ in range(40):
        health = subprocess.run([*command,'exec','-T','netbox-sync-api','python','-m','netbox_sync.web_runtime','health'],capture_output=True)
        if health.returncode == 0: break
        time.sleep(.5)
...
print('PASS '+pgmode+': real API/SOAP, trusted TLS, auth/timeout/TLS/destination/connection codes, unchanged protected state and isolated production networks',flush=True)
